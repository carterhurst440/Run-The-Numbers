-- ============================================================
-- Monkey Moonshine spins played tracking on profiles
-- Adds monkey_moonshine_spins_all_time (game_006), wires it into
-- total_progress_events / hands_played_all_time (the qualifying-events
-- grand total), and updates all related progression RPCs + the
-- sensitive-fields guard.
--
-- Mirrors supabase.fof_profile_progress.sql.
-- ============================================================

-- 1. Add the column
alter table public.profiles
  add column if not exists monkey_moonshine_spins_all_time integer not null default 0;
...
begin
  ...
  if
    ...
    new.monkey_moonshine_spins_all_time is distinct from old.monkey_moonshine_spins_all_time or
...
-- 3a. increment_profile_hands_played (now including game_006 = Monkey Moonshine)
...
begin
  ...
  set
    ...
    monkey_moonshine_spins_all_time = greatest(
      0,
      coalesce(p.monkey_moonshine_spins_all_time, 0)
      + case when coalesce(target_game_id, '') = 'game_006' then safe_increment else 0 end
    ),
...
-- 3b. increment_profile_trades_made (total now includes MM)
...
-- 3c. reconcile_profile_hands_played (counts MM resolved spins from DB)
...
begin
  ...
  set
    ...
    monkey_moonshine_spins_all_time       = coalesce(src.mm_spins, 0),
  ...
  from (
    select
      ...
      coalesce(mm.spin_count,     0)::integer as mm_spins,
      coalesce(trades.trade_count,0)::integer as trades_made
    from public.profiles p2
    left join (
      select user_id, count(*)::integer as hand_count
      from public.rtn_live_hands where status <> 'active' group by user_id
    ) rtn on rtn.user_id = p2.id
    left join (
      select user_id, count(*)::integer as hand_count
      from public.guess10_live_hands where status <> 'active' group by user_id
    ) g10 on g10.user_id = p2.id
    left join (
      select user_id, count(*)::integer as round_count
      from public.color_scheme_rounds where status = 'completed' group by user_id
    ) cs on cs.user_id = p2.id
    left join (
      select user_id, count(*)::integer as round_count
      from public.fate_or_fortune_rounds where status = 'resolved' group by user_id
    ) fof on fof.user_id = p2.id
    left join (
      select user_id, count(*)::integer as spin_count
      from public.mm_spins where status = 'resolved' group by user_id
    ) mm on mm.user_id = p2.id
    left join (
      select user_id, count(*)::integer as trade_count
      from public.shape_trader_trades group by user_id
...
-- 5. Backfill existing data from mm_spins (resolved spins)
do $$
begin
  perform set_config('rtn.allow_sensitive_balance_write', '1', true);
  ...
  update public.profiles p
  set
    monkey_moonshine_spins_all_time = coalesce(mm.spin_count, 0),
    total_progress_events = coalesce(p.run_the_numbers_hands_played_all_time, 0)
                          + coalesce(p.guess10_hands_played_all_time, 0)
                          + coalesce(p.color_scheme_rounds_played_all_time, 0)
                          + coalesce(p.fof_rounds_played_all_time, 0)
                          + coalesce(mm.spin_count, 0)
                          + coalesce(p.trades_made_all_time, 0),
