...
--
-- NOTE (10-plant model): the game runs a TEN-seed satchel (two rows of 5). Each
-- seed's fair share is targetRTP / 10 (~9.9% at 99%). (It briefly ran a 5-seed
-- model with DOUBLED pay; going back to 10 halved bloom_pay again to hold the same
-- house edge — total RTP = per-seed × N and per-seed is linear in bloom_pay, so
-- halving pay while doubling N is exact.) If you re-tune odds, re-solve pay against
-- the 10-seed / 9.9% target (the admin BLOOM editor's "Sim every flower" flags
-- drift). The live DB (admin-tuned) is authoritative; these seeds are the reset
-- baseline.
--
...
--
-- The pays were solved by exact enumeration so each of 10 seeds returns ~9.9%
-- (→ ~99% RTP). Changing a flower's odds OR super_mult requires re-solving its
-- bloom_pay against the 10-seed / 9.9% target (the admin editor flags drift).
