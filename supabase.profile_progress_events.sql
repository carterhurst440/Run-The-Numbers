...
begin
  ...
  update public.profiles as p
  set
    hands_played_all_time = greatest(0, coalesce(p.hands_played_all_time, 0) + safe_increment),
    total_progress_events = greatest(0, coalesce(p.total_progress_events, coalesce(p.hands_played_all_time, 0), 0) + safe_increment)
  where p.id = target_user_id;
...
begin
  ...
  update public.profiles as p
  set
    trades_made_all_time = greatest(
      0,
      coalesce(p.trades_made_all_time, 0) + safe_increment
    ),
    hands_played_all_time = greatest(
      0,
      coalesce(p.run_the_numbers_hands_played_all_time, 0)
